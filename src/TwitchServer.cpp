...
TwitchServer::TwitchServer(TwitchAuth&& auth) : m_auth(auth) {
    ...
    headers =
        curl_slist_append(headers, ("Client-Id: " + auth.clientID).c_str());
    headers = curl_slist_append(
        headers, ("Authorization: Bearer " + auth.accessToken).c_str());
...
std::string TwitchServer::appendUserIdsToUrl(
    std::string url,
...
std::vector<Streamer*> TwitchServer::setLiveStatus(
    std::unordered_map<std::string, Streamer>& streamers, bool streamersAdded) {
    std::string url = appendUserIdsToUrl(helixStreamsUrl, streamers);
    curl_easy_setopt(m_curl, CURLOPT_URL, url.c_str());

    std::stringstream data;
    curl_easy_setopt(m_curl, CURLOPT_WRITEDATA, (void*)&data);

    CURLcode res = curl_easy_perform(m_curl);

    if (res != CURLE_OK) {
        LOG.write(LogLevel::Error,
                  "curl failed to send get request with error: " +
    ...
    if (code != 200) {
        LOG.write(LogLevel::Error,
                  "Twitch GetStreams api call failed with error code: " +
                      std::to_string(code));
        return {};
    }
    ...
    try {
        ...
        for (auto& [user_id, streamer] : streamers) {
            ...
            auto it =
                std::find_if(liveIds.begin(), liveIds.end(),
...
size_t TwitchServer::curlWriteDataCallback(const char* contents, size_t size,
