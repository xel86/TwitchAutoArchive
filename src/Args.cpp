...
void printHelp()
{
    ...
    std::cout << "Options:\n";
    std::cout << "  -x, --debug    \tDebug mode - print additional info to log\n";
    std::cout << "  -r, --rate     \tRate at which to check if any streams have gone "
                 "live (in seconds)\n";
    std::cout << "  -c, --config   \tConfig file containing streamers to "
                 "archive and twitch credentials. Sample config can be found "
                 "in project directory.\n";
    std::cout << "  --daemon       \tFork program to run as daemon in background\n";
}
...
void parseArgs(int argc, char** argv, Args& args)
{
    ...
    for (auto it = argList.begin(), end = argList.end(); it != end; ++it)
    {
        ...
        else if (arg == "--config" || arg == "-c")
        {
            ...
            {
                std::cerr << "The file argument (-c, --config) requires a path to a "
